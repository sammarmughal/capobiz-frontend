...
export default function SolvedTickets() {
    ...
    return (
        <div>
            <table className="table table-bordered table-responsive" style={{ backgroundColor: "white" }}>
                <thead>
                    <tr>
                        <th>Title</th>
                        <th>subject</th>
                ...
                <tbody>
                    {allTickets && allTickets.filter(ticket => ticket.userId._id === user._id && ticket.status === "Open").reverse().map((ticket) => {
                        return (
                            <tr>
                                <td>{ticket.title}</td>
